	...
	class DeepStalkerTag : MonoBehaviour {
		...
		private void Update() {
			...
			if (time < targetCooldown) {
				...
				if (currentForcedTarget && time - platinumGrabTime <= 12) {
				...
				else if (!has && !currentForcedTarget && time - lastAreaCheck >= 1 && !this.GetComponent<WaterParkCreature>()) {
					lastAreaCheck = time;
					...
					WorldUtil.getGameObjectsNear(transform.position, 40, go => this.parseNearbyObject(go, loosePlatinum, stalkersWithPlatinum));
					bool flag = false;
