...
namespace ReikaKalseki.SeaToSea {
	...
	public class VoidThalassacean : CreatureAsset {
		...
		public override void AddCustomBehaviour(CreatureAsset.CreatureComponents components) {
			base.CreateTrail(GameObjectExtensions.SearchChild(this.prefab, "root", 0), new Transform[] {
				GameObjectExtensions.SearchChild(this.prefab, "spine1", 0).transform,
				GameObjectExtensions.SearchChild(this.prefab, "spine2", 0).transform,
				GameObjectExtensions.SearchChild(this.prefab, "spine3", 0).transform,
				GameObjectExtensions.SearchChild(this.prefab, "spine4", 0).transform
			}, components, 0.2f, -1f);/*
	...
	class VoidThalassaceanTag : MonoBehaviour, IOnTakeDamage {
		...
		void Update() {
			...
			if (aggressionLevel >= 1 && tryAllocateAggroToken()) {
