	...
	public class PostCoveDomeGenerator : WorldGenerator {
		...
		class ResourceDome : Spawnable {
			...
			public override GameObject GetGameObject() {
				...
				GameObject mdl = ObjectUtil.getChildObject(ObjectUtil.lookupPrefab(VanillaFlora.AMOEBOID.getPrefabID()), "lost_river_plant_04/lost_river_plant_04_membrane");
		...
		internal class ResourceDomeTag : MonoBehaviour {
			...
			void Start() {
				...
				GetComponentInChildren<Renderer>().transform.localScale = 0.3F * new Vector3(UnityEngine.Random.Range(0.9F, 1.1F), UnityEngine.Random.Range(0.9F, 1.1F), UnityEngine.Random.Range(0.6F, 1.4F));
				skies = GetComponentsInChildren<SkyApplier>(true);
				renderers = GetComponentsInChildren<Renderer>();
				InvokeRepeating("setupSky", 0, 1F);
				Invoke("refinePosition", 2.5F);
				Invoke("refinePosition", 5F);
				Invoke("refinePosition", 10F);
			...
			void setupSky() {
				...
				foreach (SkyApplier sk in skies) {
					...
					ObjectUtil.setSky(gameObject, WaterBiomeManager.main.biomeSkies[idx]);
				}
			}
